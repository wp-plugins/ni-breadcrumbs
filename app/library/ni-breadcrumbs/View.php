...
class NI_Breadcrumbs_View {
    ...
    static function make( $page, $data ) {  
        ...
        require NI_Breadcrumbs_Registry::get( 'config', 'plugin_base_dir' ) . '/app/views/' . str_replace( '.', '/', $page ) . '.phtml';
    ...
    static function statusBar( $status ) {
        ...
        foreach( $status as $stati ) :
            ...
            $message .= '<div id="message" class="' . $stati[0] . ' below-h2 ni-breadcrumbs-success">';
            $message .= '<p>' . $stati[1] . '</p>';
            $message .= '</div>';
